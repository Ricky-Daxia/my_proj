...
trading_platform:: trading_platform () { //把用户商品初始化变成类的私有函数然后调用
    ...
    if (!ifs.is_open()){
        ...
        ofs << "用户ID,用户名,密码,联系方式,地址,钱包余额,用户状态,密保问题,密码" << endl;
        ofs.close();
        user_num = 0;
    }
    else { //读取当前的用户数  
    ...
    if (!ifs.is_open()){
        ...
        sort(comment_list.begin(), comment_list.end(), [](comment*& t1, comment*& t2)->bool {return t1->num > t2->num;}); //排序
    }
    ifs.close();
...
void trading_platform:: show_menu () {
    ...
    cout << "1.管理员登录 2.用户注册 3.用户登录 4.今日新闻 5.退出程序" << endl;
    cout << "=========================================================" << endl;
    cout << endl;
}
    ...
    for (vector<user*>::iterator it = user_list.begin(); it != user_list.end(); ++it) {
        if ((*it)->user_name == user_name && (*it)->status == "正常") {
            if ((*it)->user_password == user_password) {
                ...
                else {cout << endl << "输入有误!" << endl;  Delay (1000); return {};}
            }
...
void trading_platform:: news (int i) {
    switch (i)
    {
        case 0:
        {
            cout << "2月16日晚,自由式滑雪男子空中技巧决赛,31岁的齐广璞以难度系数5.0、129分获得金牌!" << endl;
            cout <<"四战冬奥,四届世锦赛两金两银,他曾说,“最大的对手是自己”。致敬坚持与热爱!再次恭喜!" << endl;
            break;
        }
        case 1:
        {
            cout << "2月5日,中国队在北京冬奥会短道速滑混合团体接力比赛中夺冠,这是中国代表团在此次冬奥会中获得的第1枚金牌。" << endl;
            cout << "回看这场比赛：是强大实力和默契配合,塑造了速度与激情；是顽强拼搏和从容不迫,书写了光荣与梦想。整场比赛充满悬念," << endl;
            cout << "这枚金牌拿得不易!正因不易,也让拼搏更有意义!中国健儿加油!" << endl;
            break;
        }
        case 2:
        {
            cout << "2月7日晚,在北京冬奥会短道速滑男子1000米决赛中,中国选手任子威获得金牌,这是中国代表团在此次冬奥会中获得的第2枚金牌。" << endl;
            cout << "中国短道,是你永远可以相信的“国之尖刀”!" << endl;
            break;
        }
        case 3:
        {
            cout << "2月14日,在北京冬奥会自由式滑雪女子空中技巧决赛中,中国选手徐梦桃夺冠。这是中国代表团在此次冬奥会中获得的第5枚金牌。" << endl;
            cout << "4次参加冬奥首次夺得金牌!这就是热爱与坚定的力量!" << endl;
            break;
        }
        case 4:
        {
            cout << "2月12日,在北京冬奥会速度滑冰男子500米决赛中,高亭宇破纪录夺冠,赢得中国第四金。" << endl;
            cout << "此前,国家速滑馆“冰丝带”已见证多项世界纪录、奥运会纪录的诞生!" << endl;
            break;
        }
        case 5:
        {
            cout << "2月8日,在北京冬奥会自由式滑雪女子大跳台决赛,此项目是冬奥历史上第一次设置,青蛙公主谷爱凌夺得金牌!" << endl;
            cout <<"这是她自己获得的首个冬奥会奖牌!这也是中国运动员第一次参加冬奥会大跳台的比赛!" << endl;
            break;
        }
        case 6:
        {
            cout << "2月15日,北京2022年冬奥会单板滑雪男子大跳台决赛中,中国选手苏翊鸣获得金牌,帮助中国单板滑雪实现冬奥金牌零的突破!" << endl;
            cout << "这也是中国军团在本届冬奥会获得的第六块金牌,刷新了中国冬奥代表团历届最好成绩!" << endl;
            break;
        }
        default: break;
    }
